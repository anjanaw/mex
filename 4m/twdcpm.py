...
from keras.layers import Input, Dense, BatchNormalization, Conv1D, MaxPooling1D, LSTM
from keras.layers import TimeDistributed, Reshape, concatenate, Conv2D, MaxPooling2D, Flatten
...
results_file = '/home/mex/results_lopo/4m/twdcpm_.csv'
...
def build_early_fusion():
    ...
    input_twdcpm = concatenate([input_t, input_t, input_dc, input_pm], axis=2)

    x = TimeDistributed(Conv1D(32, kernel_size=5, activation='relu'))(input_twdcpm)
    ...
    model = Model(inputs=[input_t, input_w, input_dc, input_pm], outputs=x)
    model.summary()
    return model
...
def _run_(act_train_features, dc_train_features, pm_train_features, acw_train_features, train_labels,
    ...
    model.fit([act_train_features, acw_train_features, dc_train_features, pm_train_features], train_labels, verbose=1, batch_size=32, epochs=30, shuffle=True)
    ...
    results = 'act_acw_dc_pm' + ',' + str(fusion)+',' + str(i)+',' + str(accuracy)+',' + str(f_score)
    print(results)
