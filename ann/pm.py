...
results_file = '/home/mex/results_lopo/ann_pm.csv'
...
def build_shallow_model(feature_length):
...
def build_deep_model(feature_length):
    _input = Input(shape=(feature_length,))
...
def _run_(_train_features, _train_labels, _test_features, _test_labels):
    ...
    model = build_deep_model(_train_features.shape[1])
    ...
    results = 'deep' + ',' + str(accuracy)+',' + str(f_score)
    print(results)
